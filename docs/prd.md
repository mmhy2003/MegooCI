...
**API routers under `/api/v1/`:** `auth`, `projects`, `pipelines`, `builds`, `secrets-env`, `agents`, `system`, `websocket`, `search`, `roles`, `users`, `invites`, `gates`, `ai`, `registry`. Plus `GET /health` on the root app and the OCI `/v2/` endpoint tree (token auth, catalog, manifests, blobs, uploads) mounted directly on the root FastAPI app.
...
| Alembic migrations | — | ✅ | Revisions: `001_initial_schema`, `002_git_integration`, `003_agent_tokens`, `004_step_actions`, `005_roles_invites`, `006_update_role_permissions`, `007_admin_role_permissions`, `008_notification_channels`, `009_user_notifications`, `010_container_registry`. **Redundancy:** `database.init_db()` also calls `Base.metadata.create_all()` on startup, overlapping with migrations. |
...
**Pages implemented:** `/`, `/login`, `/signup`, `/dashboard`, `/pipelines`, `/pipelines/new`, `/pipelines/[id]`, `/projects`, `/projects/[id]`, `/builds`, `/builds/[id]`, `/agents`, `/secrets`, `/registry`, `/registry/[id]`, `/settings`, `/admin/users`, `/invite/accept`, `/forgot-password`, `/reset-password`, `/integrations`.
...
| Area | PRD ref | Status | Notes |
| --- | --- | --- | --- |
| **App shell — collapsible sidebar + mobile drawer** | F-9.7, F-18.7, F-18.8 | ✅ | Desktop: collapsible sidebar with state persisted to `localStorage`. Mobile: full-width off-canvas drawer with dark backdrop overlay and body scroll lock. 9 nav items (Dashboard, Pipelines, Projects, Builds, Agents, Secrets, Registry [`registry.read`-gated], Integrations [admin-only], Settings). User section at bottom with avatar dropdown (profile, theme cycle) and one-click logout with confirmation. Route change closes drawer. Dynamic breadcrumbs in header (full trail on desktop, page title + hamburger on mobile). |
...
- ❌ K8s executor, artifact flow, and notifications are still unbuilt.
