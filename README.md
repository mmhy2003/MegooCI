...
For a host-only run (no Docker, requires Go 1.22+):

```bash
make agent-bin
./agent/bin/megooci-agent run --controller ... --agent-id ... --token ...
```
...
| `agent-bin` | Build the Go agent binary on the host. |
| `agent-image` | Build the agent Docker image (no local Go needed). |
| `agent-up ID=.. TOKEN=..` | Start the agent container. |
| `agent-down` / `agent-restart` / `agent-logs` / `agent-shell` | Agent lifecycle. |
| `agent-test` / `agent-tidy` / `agent-clean` | Go module housekeeping. |
